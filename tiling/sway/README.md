...
- `config` - entry point, symlinked by `link.zsh` to `~/.config/sway/config`. Nothing but includes.
...
Only `config` is symlinked. `conf.d/` and `local/` are pulled in by absolute-path includes, so adding a file there needs no `link.zsh` change.
